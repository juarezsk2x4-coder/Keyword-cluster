...
export async function getDayBeverages(date: string): Promise<BeverageLog[]> {
  ...
  const r = await getDb().execute({ sql: `SELECT * FROM beverage_logs WHERE date = ? ORDER BY consumed_at DESC`, args: [date] });
  return r.rows as unknown as BeverageLog[];
}

export async function getDayMealLogs(date: string): Promise<MealLog[]> {
  await ensureMigrated();
...
export async function getDaySleep(date: string): Promise<SleepLog | undefined> {
  await ensureMigrated();
...
export async function getDaySubstances(date: string): Promise<SubstanceLog[]> {
  await ensureMigrated();
...
export async function getPreviousDaySubstances(date: string): Promise<SubstanceLog[]> {
  await ensureMigrated();
...
export async function getDayFatigue(date: string): Promise<boolean> {
  await ensureMigrated();
  const r = await getDb().execute({ sql: `SELECT 1 as v FROM fatigue_logs WHERE date = ?`, args: [date] });
  return r.rows.length > 0;
}

export async function getDayPrepMinutes(date: string): Promise<number | null> {
  await ensureMigrated();
